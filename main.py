...
# Si vous voulez limiter l’aventure à un canal : mettez son ID, sinon laissez None
...
async def adventure_start(interaction: discord.Interaction):
    if ADVENTURE_CHANNEL_ID and interaction.channel.id != ADVENTURE_CHANNEL_ID:
        return await interaction.response.send_message(f"❌ Use this only in <#{ADVENTURE_CHANNEL_ID}>.", ephemeral=True)
...
async def adventure_status(interaction: discord.Interaction):
    ...
    if not state:
        return await interaction.response.send_message("❌ No adventure in prog.", ephemeral=True)
...
async def adventure_end(interaction: discord.Interaction):
    ...
    if user_id in adventure_states:
        ...
        return await interaction.response.send_message("❌ Abandoned.", ephemeral=True)
...
async def on_ready():
    if GUILD_ID:
        ...
        print(f"🔄 Synced in guild {GUILD_ID}")
    else:
        await bot.tree.sync()
